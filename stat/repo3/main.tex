...
\end{align}
次に$S_j^zS_{j+1}^z$の期待値は
\begin{align}
  \begin{split}
    \Bra{\neel}S_j^zS_{j+1}^z\Ket{\neel}=&\Bra{\neel}\biggl(
      \Ket{\up}_1\otimes\cdots\otimes\frac{1}{2}\left(\subscripts{}{\Ket{\up}}{j}\subscripts{j}{\Bra{\up}}{}-\subscripts{}{\Ket{\dn}}{j}\subscripts{j}{\Bra{\dn}}{}\right)\Ket{\up}_j\\
      &\otimes\frac{1}{2}\left(\subscripts{}{\Ket{\up}}{j+1}\subscripts{j+1}{\Bra{\up}}{}-\subscripts{}{\Ket{\dn}}{j+1}\subscripts{j+1}{\Bra{\dn}}{}\right)\Ket{\dn}_{j+1}\otimes\cdots
    \biggr)\\
    =&\Bra{\neel}\left(
      \Ket{\up}_1\otimes\cdots\otimes\frac{1}{2}\Ket{\up}_j\otimes\left(-\frac{1}{2}\Ket{\dn}_{j+1}\right)\otimes\cdots
    \right)\\
    =&-\frac{1}{4}
  \end{split}
\end{align}
次に$S_j^z$の期待値は$j$が偶数のとき
\begin{align}
  \begin{split}
    \Bra{\neel}S_j^z\Ket{\neel}=&\Bra{\neel}\left(\Ket{\up}_1\otimes\cdots\otimes S_j^z\Ket{\dn}_j\otimes\cdots\right)\\
    =&-\frac{1}{2}
  \end{split}
\end{align}
$j$が奇数のとき
\begin{align}
  \begin{split}
    \Bra{\neel}S_j^z\Ket{\neel}=&\Bra{\neel}\left(\Ket{\up}_1\otimes\cdots\otimes S_j^z\Ket{\up}_j\otimes\cdots\right)\\
    =&\frac{1}{2}
  \end{split}
\end{align}
したがって$N$が偶数であることから
\begin{align}
  \Bra{\neel}\sum_{j=1}^NS_j^z\Ket{\neel}=&\frac{1}{2}-\frac{1}{2}+\cdots=0
\end{align}
以上からエネルギーの期待値は
\begin{align}
  \Braket{\neel|H|\neel}=0+\sum_{j=1}^N\left(-\frac{1}{4}\right)-0=-\frac{J\Delta N}{4}
\end{align}
したがって1スピンあたりのエネルギー期待値は
\begin{align}
  e_N=\frac{\Braket{\neel|H|\neel}}{N}=-\frac{J\Delta}{4}
\end{align}
である.
次に$e_N=e_0(h_c)$となる$h_c$を求める.
